...
## Preconfigured page
If you don't want to spend time reading through all the available configuration options and just want something to get you going quickly you can use [this `bonjour.yml` file](bonjour.yml) and make changes to it as you see fit. It will give you a page that looks like the following:
...
> [!NOTE]
>
> If you attempt to start Bonjour with an invalid config it will exit with an error outright. If you successfully started Bonjour with a valid config and then made changes to it which result in an error, you'll see that error in the console and Bonjour will continue to run with the old configuration. You can then continue to make changes and when there are no errors the new configuration will be loaded.
...
Inserting environment variables is supported anywhere in the config. This is done via the `${ENV_VAR}` syntax. Attempting to use an environment variable that doesn't exist will result in an error and Bonjour will either not start or load your new config on save. Example:
...
```sh
bonjour --config /path/to/bonjour.yml config:print | less -N
```

This is a bit more convoluted when running Bonjour inside a Docker container:

```sh
docker run --rm -v ./bonjour.yml:/app/config/bonjour.yml ghcr.io/dishine-digital-agency/bonjour config:print | less -N
```

This assumes that the config you want to print is in your current working directory and is named `bonjour.yml`.
...
For property descriptions, validation and autocompletion of the config within your IDE, @not-first has created a [config schema](https://github.com/not-first/glance-schema) that is compatible with Bonjour's YAML format. Go check it out and give them a star!
...
```sh
docker run --rm ghcr.io/dishine-digital-agency/bonjour secret:make
```
...
```sh
./bonjour password:hash mysecretpassword
```

Or with Docker:

```sh
docker run --rm ghcr.io/dishine-digital-agency/bonjour password:hash mysecretpassword
```
...
Bonjour will automatically block IP addresses of users who fail to authenticate 5 times in a row in the span of 5 minutes. In order for this feature to work correctly, Bonjour must know the real IP address of requests. If you're using a reverse proxy such as nginx, Traefik, NPM, etc, you must set the `proxied` property in the `server` configuration to `true`:
...
When set to `true`, Bonjour will use the `X-Forwarded-For` header to determine the original IP address of the request, so make sure that your reverse proxy is correctly configured to send that header.
...
server:
  port: 8080
  assets-path: /home/user/bonjour-assets
```
...
#### `proxied`
Set to `true` if you're using a reverse proxy in front of Bonjour. This will make Bonjour use the `X-Forwarded-*` headers to determine the original request details.

#### `base-url`
The base URL that Bonjour is hosted under. No need to specify this unless you're using a reverse proxy and are hosting Bonjour under a directory. If that's the case then you can set this value to `/bonjour` or whatever the directory is called. Note that the forward slash (`/`) in the beginning is required unless you specify the full domain and path.

> [!IMPORTANT]
> You need to strip the `base-url` prefix before forwarding the request to the Bonjour server.
...
> ```
> /home/user/bonjour-assets
> ```
>
> You should mount:
> ```
> /home/user/bonjour-assets:/app/assets
> ```
>
...
Say you have a directory `bonjour-assets` with a file `gitea-icon.png` in it and you specify your assets path like:
...
branding:
  custom-footer: |
    <p>Powered by <a href="https://github.com/diShine-digital-agency/bonjour">Bonjour</a></p>
...
| app-name | string | no | Bonjour |
| app-icon-url | string | no | Bonjour's default icon |
| app-background-color | string | no | Bonjour's default background color |
...
> [!TIP]
>
> Because Bonjour uses a lot of utility classes it might be difficult to target some elements. To make it easier to style specific widgets, each widget has a `widget-type-{name}` class, so for example if you wanted to make the links inside just the RSS widget bigger you could use the following selector:
>
> ```css
> .widget-type-rss a {
>     font-size: 1.5rem;
...
###### `item-link-prefix`
If an RSS feed isn't returning item links with a base domain and Bonjour has failed to automatically detect the correct domain you can manually add a prefix to each link with this property.
...
> [!WARNING]
>
> Reddit does not allow unauthorized API access from VPS IPs, if you're hosting Bonjour on a VPS you will get a 403
...
> (see `proxy` property) or route the traffic from Bonjour through a VPN.
...
A custom request URL that will be used to fetch the data. This is useful when you're hosting Bonjour on a VPS where Reddit is blocking the requests and you want to route them through a proxy that accepts the URL as either a part of the path or a query parameter.
...
##### `proxy`
A custom HTTP/HTTPS proxy URL that will be used to fetch the data. This is useful when you're hosting Bonjour on a VPS where Reddit is blocking the requests and you want to bypass the restriction by routing the requests through a proxy. Example:
...
<details>
<summary>View <code>bonjour.yml</code></summary>
<br>
...
> [!NOTE]
>
> The configuration of this widget requires some basic knowledge of programming, HTML, CSS, the Go template language and Bonjour-specific concepts.
...
##### `url`
The URL to fetch the data from. It must be accessible from the server that Bonjour is running on.
...
##### `location`
The name of the city and country to fetch weather information for. Attempting to launch the application with an invalid location will result in an error. You can use the [geocoding API page](https://open-meteo.com/en/docs/geocoding-api) to search for your specific location. Bonjour will use the first result from the list if there are multiple.
...
The URL of the monitored service, which must be reachable by Bonjour, and will be used as the link to go to when clicking on the title. If `check-url` is not specified, this is used as the status check.
...
- type: releases
  ...
  repositories:
    - go-gitea/gitea
    - jellyfin/jellyfin
    - diShine-digital-agency/bonjour
...
repositories:
  ...
  - dockerhub:diShine-digital-agency/bonjour
  - codeberg:redict/redict
```
...
repositories:
  - gitlab:inkscape/inkscape
  - repository: diShine-digital-agency/bonjour
...
You can also specify the value for this token through an ENV variable using the syntax `${GITHUB_TOKEN}` where `GITHUB_TOKEN` is the name of the variable that holds the token. If you've installed Bonjour through docker you can specify the token in your docker-compose:
...
and then use it in your `bonjour.yml` like this:
...
This way you can safely check your `bonjour.yml` in version control without exposing the token.
...
> [!NOTE]
>
> The widget requires access to `docker.sock`. If you're running Bonjour inside a container, this can be done by mounting the socket as a volume:
>
> ```yaml
> services:
>   bonjour:
...
```yaml
  jellyfin:
    ...
    labels:
      bonjour.name: Jellyfin
      bonjour.icon: si:jellyfin
      bonjour.url: https://jellyfin.domain.com
      bonjour.description: Movies & shows
```

Alternatively, you can also define the values within your `bonjour.yml` via the `containers` property, where the key is the container name and each value is the same as the labels but without the "bonjour." prefix:
...
For services with multiple containers you can specify a `bonjour.id` on the "main" container and `bonjour.parent` on each "child" container:
...
services:
  immich-server:
    ...
    labels:
      bonjour.name: Immich
      bonjour.icon: si:immich
      bonjour.url: https://immich.domain.com
      bonjour.description: Image & video management
      bonjour.id: immich

  redis:
    image: docker.io/redis:6.2-alpine
    labels:
      bonjour.parent: immich
      bonjour.name: Redis
...
##### `hide-by-default`
Whether to hide the containers by default. If set to `true` you'll have to manually add a `bonjour.hide: false` label to each container you want to display. By default all containers will be shown and if you want to hide a specific container you can add a `bonjour.hide: true` label.
...
###### `category`
Filter to only the containers which have this category specified via the `bonjour.category` label. Useful if you want to have multiple containers widgets, each showing a different set of containers.
...
services:
  jellyfin:
    ...
    labels:
      ...
      bonjour.icon: si:jellyfin
      bonjour.url: https://jellyfin.domain.com
      bonjour.category: media

  gitea:
    image: gitea/gitea:latest
    labels:
      bonjour.name: Gitea
      bonjour.icon: si:gitea
      bonjour.url: https://gitea.domain.com
      bonjour.category: dev-tools

  vaultwarden:
    image: vaultwarden/server:latest
    labels:
      bonjour.name: Vaultwarden
      bonjour.icon: si:vaultwarden
      bonjour.url: https://vaultwarden.domain.com
      bonjour.category: dev-tools
```
...
| Name | Description |
| ---- | ----------- |
| bonjour.name | The name displayed in the UI. If not specified, the name of the container will be used. |
| bonjour.icon | See [Icons](#icons) for more information on how to specify icons |
| bonjour.url | The URL that the user will be redirected to when clicking on the container. |
| bonjour.same-tab | Whether to open the link in the same or a new tab. Default is `false`. |
| bonjour.description | A short description displayed in the UI. Default is empty. |
| bonjour.hide | Whether to hide the container. If set to `true` the container will not be displayed. Defaults to `false`. |
| bonjour.id | The custom ID of the container. Used to group containers under a single parent. |
| bonjour.parent | The ID of the parent container. Used to group containers under a single parent. |
| bonjour.category | The category of the container. Used to filter containers by category. |
...
### Server Stats
Display statistics such as CPU usage, memory usage and disk usage of the server Bonjour is running on or other servers.
...
To display data from a remote server you need a compatible monitoring agent running on that server. An open-source agent that works with Bonjour is available [here](https://github.com/glanceapp/agent) — keep in mind that it is still in development and may not work as expected. Support for additional providers will be added in the future.
...
##### `servers`
If not provided it will display the statistics of the server Bonjour is running on.
...
This is useful if you're running Bonjour inside of a container which usually mounts a lot of irrelevant filesystems.
