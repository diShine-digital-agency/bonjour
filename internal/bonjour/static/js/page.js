...
async function refreshPageContent() {
    ...
    try {
        const content = await fetchPageContent(pageData);
...
function setupAutoRefresh() {
    const refreshMeta = document.querySelector('meta[name="bonjour-auto-refresh"]');
    if (!refreshMeta) return;

    const intervalMinutes = parseInt(refreshMeta.getAttribute("content"), 10);
    if (isNaN(intervalMinutes) || intervalMinutes < 1) return;

    const intervalMs = intervalMinutes * 60 * 1000;
    setInterval(refreshPageContent, intervalMs);
}
